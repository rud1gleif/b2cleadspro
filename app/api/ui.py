...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .tag button { background: none; border: none; color: #fff; cursor: pointer;
                font-size: 14px; line-height: 1; padding: 0; }
  .tags-wrap input { background: none; border: none; outline: none; color: var(--text);
                     font-size: 13px; min-width: 100px; flex: 1; }

  input[type=text], input[type=number], textarea {
    background: var(--surface2); border: 1px solid var(--border); border-radius: 8px;
    color: var(--text); font-size: 13px; padding: 8px 10px; width: 100%;
...
<aside id="sidebar">
  ...
  <div class="row2">
    <div class="field">
      ...
      <input type="number" id="max-pages" value="10" min="1" max="500">
    </div>
    <div class="field">
...
locInput.addEventListener('keydown', e => {
  if ((e.key === 'Enter' || e.key === ',') && locInput.value.trim()) {
    e.preventDefault();
    addLocation(locInput.value.trim().replace(/,$/, ''));
    locInput.value = '';
...
function getActiveSources() {
  return [...document.querySelectorAll('.src-btn.active')].map(b => b.dataset.src);
}
...
function updateJobMeta(job) {
  ...
  document.getElementById('job-meta').textContent =
    `${job.leads_found} leads \u00b7 ${job.status} \u00b7 sources: ${job.sources}`;
  loadJobs();
}
...
async function loadJobs() {
  ...
  jobs.slice().reverse().forEach(job => {
    ...
    div.className = 'job-card' + (job.id === currentJobId ? ' active' : '');
    div.dataset.id = job.id;
    div.onclick = () => loadJob(job.id);
    div.innerHTML = `
      <div class="job-card-top">
        <span class="job-id">#${job.id}</span>
        <span class="badge ${job.status}">${job.status.toUpperCase()}</span>
      </div>
      <div class="job-meta">${locs} &middot; ${job.leads_found} leads</div>`;
...
function exportCSV() {
  ...
  a.download = `leads_job_${currentJobId}.csv`;
  a.click();
}
